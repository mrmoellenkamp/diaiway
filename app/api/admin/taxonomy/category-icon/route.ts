...
/** Admin: Icon-Bild für eine Taxonomy-Kategorie (Vercel Blob). */
export async function POST(req: Request) {
  ...
  try {
    ...
    const categoryId = (formData.get("categoryId") as string | null) ?? "new"

    if (!file) {
      return NextResponse.json({ error: "Keine Datei." }, { status: 400 })
    }
    if (!ALLOWED_TYPES.includes(file.type)) {
      return NextResponse.json({ error: "Nur JPG, PNG, WebP, GIF." }, { status: 400 })
    }
    ...
    const filename = `taxonomy/category-${categoryId}-${Date.now()}.${ext}`
    const blob = await put(filename, buffer, { access: "public", contentType })

    return NextResponse.json({ url: blob.url })
  } catch (err: unknown) {
    console.error("[category-icon POST]", err)
