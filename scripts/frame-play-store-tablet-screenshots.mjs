/**
 * Erzeugt **Tablet**-Screenshots für Google Play (Landscape), getrennt von Phone-Assets.
 *
 * Play Console (Tablet-Sektion): kurze Kante mindestens 1080 px, lange Kante max. 7680 px;
 * üblich z. B. **1920×1200** (16:10). Mindestens 2 Screenshots – bei Large-Screen-/Tablet-Support
 * in der Regel **Pflicht**, nicht optional.
 *
 * Nutzung: node scripts/frame-play-store-tablet-screenshots.mjs
 * Eingabe:  assets/Screenshot_*.png (ideal: Aufnahmen vom Tablet-Emulator / -Gerät im Querformat)
 * Ausgabe: assets/play-store-screenshots-tablet/*.png
 *
 * Hinweis: Portrait-Phone-Screenshots werden per Cover zugeschnitten – für echte Tablet-Listings
 * besser direkt im Tablet-Layout aufnehmen.
 */
...
const OUT_DIR = path.join(ROOT, "assets", "play-store-screenshots-tablet")
...
async function processPng(filename) {
  ...
  const outName = filename.replace(/\.png$/i, "-1920x1200.png")
...
async function main() {
  ...
  console.log(`${files.length} Screenshot(s) → ${OUT_W}×${OUT_H} px (Tablet / Landscape)\n`)
