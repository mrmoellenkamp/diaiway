...
| **Pflicht vor Zahlung** | Vollständigkeit nach `lib/invoice-requirements.ts` (`validateInvoiceDataForPayment`): **Privat** — vollständiger Name, Anschrift, PLZ, Ort, Land, E-Mail; **Unternehmen** — Firmenname + dieselbe Anschrift/E-Mail, vollständiger Name **optional**. **Optional:** USt-IdNr., Steuernummer. | Server prüft Vollständigkeit der **gespeicherten** `invoiceData` **vor** jedem Zahlungsstart (`getInvoiceGateResult` / `assertInvoiceCompleteForPayment` in API-Routen; Stripe-Embedded-Actions `startBookingCheckout` / `startSessionCheckout` geben bei Blockade `ok: false` zurück). Ohne diese Daten **keine** Zahlung. |
| **Checkout-Platzhalter** | `POST /api/bookings` legt sofort eine Zeile an (`status: pending`, `paymentStatus: unpaid`, `deferNotification`); Takumi wird erst nach Zahlung informiert. | Sessions-UI blendet `scheduled` + `pending` + `unpaid` aus (`lib/booking-display.ts`); Housekeeping storniert solche Einträge nach 24 h (`cancelAbandonedUnpaidScheduledBookings` via `runBookingListHousekeeping`). Takumi: „Antworten“ nur bei `instant` oder nach `paid`. |
